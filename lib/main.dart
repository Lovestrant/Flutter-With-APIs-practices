...
import 'package:flutter_apis/DataModel.dart';
...
class _MyHomePageState extends State<MyHomePage> {
 ...
 Future<DataModel?> submitData(String name, String job) async {
    var response = await http.post(Uri.https('regres.in', 'api/users'),
        body: {"name": name, "job":job}
      );
    String resFromJson = response.body;

    print(response.statusCode);
  if(response.body == 201) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text("Res is: okay"),
    ));
  }
  else {
    return null;
  ...
  Future apiCall() async {
    ...
    if(response.statusCode == 200) {
     // dataFromApi = response.body.toString();
